...
How to use lifecycle hooks for quality validation in DeepWork jobs.
...
Create `.deepwork/jobs/[job_name]/hooks/validate.sh`:
...
Make it executable:
```bash
chmod +x .deepwork/jobs/[job_name]/hooks/validate.sh
```
...
## Deprecated: stop_hooks

The `stop_hooks` field is deprecated but still supported:
...
Internally, `stop_hooks` are merged into `hooks.after_agent`.
