...
- **Everything else** (refactors, style changes, architectural suggestions, anything with trade-offs): use AskUserQuestion to ask the user about each finding **individually**. Do not group issues together unless they are the same issue occurring in multiple files; otherwise, use AskUserQuestion to ask about each issue separately. This lets the user make separate decisions on each item. For each question, provide several concrete fix approaches as options when there are reasonable alternatives (e.g., "Update the spec to match the code" vs "Update the code to match the spec" vs "Skip"). If any of the issues seem minor to you and like ones that you are not sure should be raised again, include an option in the list that is a good approach to suppress that error in the future. It could be a clarification to the rule if the rule is narrow, it could be adding a comment to the offending content if comments work, etc. Be concise — quote the key finding, don't dump the full review.
