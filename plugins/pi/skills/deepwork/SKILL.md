...
Prefer direct DeepWork MCP tools when Pi exposes them. If they are not visible, use the
`mcp` proxy tool from `pi-mcp-adapter`: search for the needed tool first, then call the
returned tool name with JSON-string arguments.

Examples:
- Search: `mcp({ search: "get_workflows" })`
- Call: `mcp({ tool: "<returned_get_workflows_tool_name>", args: "{}" })`

If no DeepWork MCP server is available, ask the user to run `/deepwork_setup`, then `/reload`.
...
If the user invokes `/deepwork learn` and `/record` was used earlier in this
