...
When the user invokes the `deepwork` skill:
...
2. Based on the available flows and what the user said in their request, proceed:
   - **Explicit workflow**: `deepwork <a workflow name>` -> start that workflow
   - **General request**: `deepwork <a request>` -> infer the best match from available workflows
   - **No context**: `deepwork` alone -> ask the user to choose from available workflows
