...
- **Review generation** - schemas automatically generate review rules that run during review flows and workflow quality gates
...
During review runs and workflow quality gates, each schema generates a review rule that checks all requirements.
...
2. DeepWork built-in standard schemas (for example `job_yml`, `deepschema`)
