...
class TestFormatForGemini:
    """Tests for format_for_gemini."""

    def test_empty_tasks_returns_no_tasks_message(self, tmp_path: Path) -> None:
        result = format_for_gemini([], tmp_path)
        assert "No review tasks" in result

    def test_output_starts_with_sequential_header(self, tmp_path: Path) -> None:
        task = _make_task()
        file_path = tmp_path / ".deepwork" / "tmp" / "review_instructions" / "123.md"
        file_path.parent.mkdir(parents=True)
        file_path.write_text("content")
        result = format_for_gemini([(task, file_path)], tmp_path)
        assert result.startswith("Execute the following reviews sequentially:")

    def test_review_names_present(self, tmp_path: Path) -> None:
        task = _make_task(rule_name="py_review", files=["src/app.py"])
        file_path = tmp_path / "instructions.md"
        result = format_for_gemini([(task, file_path)], tmp_path)
        assert "py_review review of src/app.py" in result

    def test_grouped_task_name_includes_file_count(self, tmp_path: Path) -> None:
        task = _make_task(rule_name="py_review", files=["a.py", "b.py", "c.py"])
        file_path = tmp_path / "instructions.md"
        result = format_for_gemini([(task, file_path)], tmp_path)
        assert "py_review review of 3 files" in result
    ...
    def test_instruction_file_reference(self, tmp_path: Path) -> None:
        ...
        file_path.write_text("content")
        result = format_for_gemini([(task, file_path)], tmp_path)
        assert "Instruction file: .deepwork/tmp/review_instructions/7142141.md" in result

    def test_numbered_sequentially(self, tmp_path: Path) -> None:
        task_a = _make_task(rule_name="rule_a", files=["a.py"])
        task_b = _make_task(rule_name="rule_b", files=["b.py"])
        file_a = tmp_path / "a.md"
        file_b = tmp_path / "b.md"
        result = format_for_gemini([(task_a, file_a), (task_b, file_b)], tmp_path)
        assert "1. **rule_a review of a.py**" in result
        assert "2. **rule_b review of b.py**" in result

    def test_scope_prefix_from_subdirectory_source(self, tmp_path: Path) -> None:
        """Rules from subdirectory .deepreview files include scope prefix."""
        task = _make_task(rule_name="job_definition_review", files=["job.yml"])
        task.source_location = "jobs/my_job/.deepreview:1"
        file_path = tmp_path / "instructions.md"
        result = format_for_gemini([(task, file_path)], tmp_path)
        assert "my_job/job_definition_review review of job.yml" in result
        assert "Description: Review my_job/job_definition_review" in result
    ...
    def test_no_scope_prefix_for_root_level_source(self, tmp_path: Path) -> None:
        ...
        file_path = tmp_path / "instructions.md"
        result = format_for_gemini([(task, file_path)], tmp_path)
        assert "py_review review of app.py" in result
        assert "Description: Review py_review" in result
